...
#define QUIT_LEN            7
const char simulationTime[] = "1 ms";
const char codeDir[] = "code/290";
const char MarsDir[] = "MARS_290.jar";
...
int main() {
    ...
    for (int i = TEST_BEGIN_NUM; i < TEST_END_NUM; i++) {
        ...
        sprintf(asmFileName, "%s/testpoint%d.asm", codeDir, i);
        ...
        system(buffer1);
        sprintf(buffer1, "java -jar %s %s nc mc CompactDataAtZero db >%s", MarsDir, asmFileName, masterName);
        system(buffer1);
